...
def solve(top_row, i, N, c1, c2, c3, brck_amt, connection, wall):
    if wall == brck_amt:
        return True
    if wall > brck_amt:
        return False
    ...
    for _ in range(N):
        if c1 > 0:
            if solve(top_row, i + 1, N, c1 - 1,
                     c2, c3, brck_amt, connection, wall + 1):
                return True
        if c3 > 0:
            if solve(top_row, i + 1, N, c1, c2,
                     c3 - 1, brck_amt, connection, wall + 3):
                return True
        if c2 > 0:
            if solve(top_row, i + 1, N, c1,
                     c2 - 1, c3, brck_amt, connection, wall + 2):
                return True
